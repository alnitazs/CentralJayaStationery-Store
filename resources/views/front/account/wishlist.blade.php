            ...
            <div class="col-md-9">
                <div class="card">
                    <div class="card-header">
                        <h2 class="h5 mb-0 pt-2 pb-2">Wishlist Saya</h2>
                    </div>
                            ...
                            <div class="d-block d-sm-flex align-items-start text-center text-sm-start">
                                ...
                                <div class="pt-2">
                                    ...
                                    <div class="fs-lg text-accent pt-2">
                                        <span class="h5 fw-bold text-dark">Rp {{ number_format($wishlist->product->price * 1000, 0, ',', '.') }}</span>
                                        @if($wishlist->product->compare_price > 0)
                                        <span class="h6 text-muted text-decoration-line-through ms-2">Rp {{ number_format($wishlist->product->compare_price * 1000, 0, ',', '.') }}</span>
                                        @endif
                                    </div>
                                </div>
                            </div>
                            <div class="pt-2 ps-sm-3 mx-auto mx-sm-0 text-center">
                                <button onclick="removeProduct({{ $wishlist->product_id }})" class="btn btn-outline-danger btn-sm" type="button"><i class="fas fa-trash-alt me-2"></i>Hapus</button>
                            </div>
                        </div>
                        @endforeach
                        @else
                        <div>
                            <h3 class="h5">Wishlist Anda kosong!</h3>
                        </div>
                        @endif
                    </div>
                </div>
